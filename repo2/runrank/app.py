...
# SQL placeholder (SQLite: ?, Postgres: %s)
PH = "%s" if IS_POSTGRES else "?"

# SQLite 기본 경로(로컬/개발용). Postgres를 쓰면 무시됨.
...
def db():
    ...
    - Render 이벤트(5일)용: Render Postgres Free + DATABASE_URL 사용 권장
    - 로컬: SQLite 사용
    """
    if IS_POSTGRES:
        if psycopg is None:
            raise RuntimeError("psycopg가 설치되지 않았습니다. requirements.txt에 psycopg[binary]를 추가하세요.")
        # Render 내부망/외부망 URL 둘 다 사용 가능
...
def submit_vote(body: SubmitBody):
    ...
    cur.execute(
        f"SELECT id, run_id FROM vote_batches WHERE user={PH} AND vote_day_kst={PH} LIMIT 1",
        (user, vote_day_kst),
    )
    ...
    if IS_POSTGRES:
        cur.execute(
            f"INSERT INTO vote_batches(user, vote_day_kst, ts_epoch, run_id) VALUES({PH}, {PH}, {PH}, NULL) RETURNING id",
            (user, vote_day_kst, ts_epoch),
        )
        batch_id = cur.fetchone()["id"]
    else:
        cur.execute(
            f"INSERT INTO vote_batches(user, vote_day_kst, ts_epoch, run_id) VALUES({PH}, {PH}, {PH}, NULL)",
            (user, vote_day_kst, ts_epoch),
        )
    ...
    for tid in track_ids:
        if IS_POSTGRES:
            cur.execute(
                f"INSERT INTO submissions(track_id, user, ts_epoch, vote_day_kst) VALUES({PH}, {PH}, {PH}, {PH}) ON CONFLICT DO NOTHING",
                (tid, user, ts_epoch, vote_day_kst),
            )
        else:
            cur.execute(
                f"INSERT OR IGNORE INTO submissions(track_id, user, ts_epoch, vote_day_kst) VALUES({PH}, {PH}, {PH}, {PH})",
                (tid, user, ts_epoch, vote_day_kst),
            )
        if cur.rowcount == 1:
            inserted += 1
...
def try_auto_vote_tracks(user: str, track_ids: List[int], run_id: Optional[int] = None) -> Dict[str, Any]:
    ...
    if IS_POSTGRES:
        cur.execute(
            f"INSERT INTO vote_batches(user, vote_day_kst, ts_epoch, run_id) VALUES({PH}, {PH}, {PH}, {PH}) RETURNING id",
            (user, vote_day_kst, ts_epoch, run_id),
        )
        batch_id = cur.fetchone()["id"]
    else:
        cur.execute(
            f"INSERT INTO vote_batches(user, vote_day_kst, ts_epoch, run_id) VALUES({PH}, {PH}, {PH}, {PH})",
            (user, vote_day_kst, ts_epoch, run_id),
        )
        batch_id = cur.lastrowid
...
async def create_run(
    ...
    for tid in parsed_track_ids:
        if IS_POSTGRES:
            ...
            cur.execute(
                f"INSERT OR IGNORE INTO run_tracks(run_id, track_id) VALUES({PH}, {PH})",
                (run_id, tid),
            )
    conn.commit()
    conn.close()
...
def list_runs(user: Optional[str] = None, limit: int = 30):
    ...
    if user and user.strip():
        cur.execute(f"SELECT * FROM runs WHERE user={PH} ORDER BY ts_epoch DESC LIMIT {PH}", (user.strip(), n))
    else:
    ...
    if run_ids:
        if IS_POSTGRES:
            cur.execute(
                "SELECT run_id, track_id FROM run_tracks WHERE run_id = ANY(%s) ORDER BY run_id",
                (run_ids,),
            )
        else:
